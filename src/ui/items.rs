...
const ARMOR_DATA: &[(i32, i32, i32, &str)] = &[
...
pub const ARMOR_CATEGORY: i32 = 0x10000000;
...
impl Items {
    ...
    pub fn get_armor_data() -> &'static [(i32, i32, i32, &'static str)] {
        ARMOR_DATA
    }
    ...
    pub fn get_item(
        ...
        if category == ITEMS_CATEGORY {
            ...
            return None;
        } else if category == ARMOR_CATEGORY {
            for &(id, max_count, _, name) in ARMOR_DATA.iter() {
                if id == item_id {
                    let count = if item_count > max_count {
                        max_count
                    } else {
    ...
    pub fn execute_get_item(
        ...
        println!(
            "Inventory context (resolved + offset): {:#x}",
            inventory_context
        );
